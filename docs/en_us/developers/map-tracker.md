...
- `rotation_speed`: Positive real number, default `2.75`. The rotation speed multiplier when the player adjusts the orientation, in pixels per degree. A larger value adjusts the orientation faster but may cause over-adjustment and repeated adjustments; a smaller value adjusts the orientation smoothly but may cause delayed adjustment.
...
- `sprint_threshold`: Positive real number, default `15.0`. The distance threshold for performing the sprint action, in pixel distance. When the distance between the player and the next target point exceeds this value and the orientation is correct, the player will perform a sprint.
